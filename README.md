
<h1 align="center">Hi 👋, I'm Aditya Rana</h1>
<h3 align="center">Full Stack Developer | Competitive Programmer | Cloud Enthusiast</h3>

---

### 👨‍💻 About Me
- 🎓 B.Tech in **Electrical Engineering** — BIT Sindri  
- 💼 **Full Stack Developer at Capgemini** with **2+ years of professional experience**
- ⭐ **3★ Coder on CodeChef**
- 🔥 Solved **500+ DSA problems** on LeetCode  
- 🧠 Strong interest in **System Design**, **Cloud Computing**, and **High‑performance Applications**
- 💡 Passionate about building **scalable**, **maintainable**, and **clean software**

---

### 🏆 Certifications & Achievements
- ☁️ **Microsoft Certified: Azure Fundamentals (AZ‑900)**
- 🤖 **Microsoft Certified: AI Fundamentals (AI‑900)**
- 🏗️ **Microsoft Exam AZ‑305 Passed** (Designing Microsoft Azure Infrastructure Solutions)
- 💻 **500+ LeetCode Questions Solved**
- ⭐ **3★ CodeChef Competitive Programmer**

---


### 🔗 Connect with me

- 🍜 **CodeChef:** [its_aadi](https://www.codechef.com/users/its_aadi)
- 🟧 **LeetCode:** [its_aadi](https://leetcode.com/u/its_aadi/) 

---

### 🛠️ Tech Stack

#### **Languages**
- Java | C# | C++ | JavaScript | TypeScript | SQL

#### **Frontend**
- React.js | HTML5 | CSS3 | TailwindCSS | Bootstrap

#### **Backend**
- .Net | Spring Boot | Express.js 

#### **Databases**
- MySQL | PostgreSQL | MongoDB

#### **Tools & Platforms**
- Git | GitHub | Docker | Azure | Postman | Linux  
- CI/CD Basics | REST API Development | Agile

---

### 📊 GitHub Stats
<p align="center">
  https://github-readme-stats.vercel.app/api?username=Aadi-404&show_icons=true&theme=tokyonight
  https://github-readme-stats.vercel.app/api/top-langs/?username=Aadi-404&layout=compact&theme=tokyonight
</p>

---

### 🌱 Currently Exploring
- Cloud‑native Architecture  
- Microservices & Distributed Systems  
- Competitive Programming (advanced problem solving)

---


### 🚀 Quote I Follow
> _"First, solve the problem. Then, write the code."_

---

⭐ **Feel free to explore my repositories and connect for collaboration!**
